...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    float charF = -3.5 + 8.0 * noise(id + 6.0 * charPos);
    charPos = fract(charPos);
    charF *= smoothstep(0.0, 0.4, charPos.x) * smoothstep(1.0, 0.6, charPos.x);
    charF *= smoothstep(0.0, 0.4, charPos.y) * smoothstep(1.0, 0.6, charPos.y);
    color = mix(color, signColor, flash * flicker * step(outline, 0.01) * clamp(charF, 0.0, 1.0));
